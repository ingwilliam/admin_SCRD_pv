            ...
            if ($("#convocatoria").val() != "")
            {
                if($("#busqueda").val() == "0")
                {
                    ...
                    $('#table_list').DataTable({
                        ...
                        "lengthMenu": [20, 30, 40],
                        "ajax": {
                            url: url_pv + "PropuestasAdministracion/buscar_propuestas",
                            data: function (d) {
            ...
            } else
            {
                if ($("#entidad").val() != "")
                {
                    $.ajax({
                        ...
                        url: url_pv + 'PropuestasAdministracion/select_convocatorias'
                    }).done(function (data) {
                        if (data == 'error_metodo')
                        {
        ...
        $('#convocatoria').change(function () {
            ...
            if ($("#convocatoria").val() != "")
            {
                $.ajax({
                    ...
                    url: url_pv + 'PropuestasAdministracion/select_categorias'
                }).done(function (data) {
                    if (data == 'error_metodo')
                    {
                        notify("danger", "ok", "Usuarios:", "Se registro un error en el método, comuníquese con la mesa de ayuda soporte.convocatorias@scrd.gov.co");
...
function cargar_propuesta(token_actual)
{
    ...
    $(".cargar_propuesta").click(function () {
        ...
        $.ajax({
            ...
            url: url_pv + 'PropuestasAdministracion/cargar_propuesta/'+title
        }).done(function (data) {
            if (data == 'error_metodo')
            {
                notify("danger", "ok", "Convocatorias:", "Se registro un error en el método, comuníquese con la mesa de ayuda soporte.convocatorias@scrd.gov.co");
...
function cargar_tabla_archivos(token_actual, documento) {
    ...
    $.ajax({
        type: 'GET',
        data: {documento: documento, "token": token_actual.token, modulo: "Menu Participante", propuesta: $("#propuesta").attr('value')},
        url: url_pv + 'PropuestasAdministracion/buscar_archivos'
    }).done(function (data) {
        if (data == 'error_metodo')
        {
            notify("danger", "ok", "Convocatorias:", "Se registro un error en el método, comuníquese con la mesa de ayuda soporte.convocatorias@scrd.gov.co");
...
function cargar_tabla_link(token_actual, documento) {
    ...
    $.ajax({
        type: 'GET',        
        data: {documento: documento, "token": token_actual.token, modulo: "Menu Participante", propuesta: $("#propuesta").attr('value')},
        url: url_pv + 'PropuestasAdministracion/buscar_link'
    }).done(function (data) {
        if (data == 'error_metodo')
        {
            notify("danger", "ok", "Convocatorias:", "Se registro un error en el método, comuníquese con la mesa de ayuda soporte.convocatorias@scrd.gov.co");
